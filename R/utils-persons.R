...
as_person_bibtex_comments <- function(x) {
  # Can extract comments as R
  comments_list <- NULL
  locate_comment <- regexpr("<|\\(", x)
  start_comment <- min(unlist(locate_comment))

  if (start_comment > 0) {
    # Has comments
    the_comment <- trimws(substr(x, start_comment, nchar(x)))

    # Let R parse comments
    res <- hint_person(paste0("fakename ", the_comment))[[1]]
    comments_list <- res[names(res) != "name"]

    x <- trimws(substr(x, 0, start_comment - 1))
  }
  ...
  if (!is.null(comments_list)) {
    end_list <- c(end_list, comments_list)
  }
