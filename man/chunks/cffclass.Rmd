Under the hood, a [`cff`] object is a named [`list`] with additional methods,
most notably [`print()`] and [`as_cff()`].
...
[`as_cff()`] converts a [`list`] to [`cff`] and removes elements that are `NULL`
or `NA`. Named elements are known as keys in CFF terminology.

### Subclasses

\CRANpkg{cffr} implements two special subclasses of [`cff`], which represent
two types of objects defined in the [Citation File Format
...
- `definitions.person` and `definitions.entity`: CFF definitions for sublists
  representing persons or entities. In \CRANpkg{cffr}, the subclass
  [`cff_pers_lst`] collects an array of `definitions.person` or
  `definitions.entity` objects, where each individual person or entity has
  subclass [`cff_pers`].

- `definitions.reference`: The CFF definition for references to related works
  and other software used in the main work described by the `CITATION.cff` file.
  In \CRANpkg{cffr}, the subclass [`cff_ref_lst`] represents arrays of
  `definitions.reference` objects, where each element has subclass
  [`cff_ref`].

These subclasses are not complete, valid [`cff`] objects. They adapt
information to the structures required by the CFF schema.

The examples below use [`utils::person()`].
...
Creating a [`cff`] object does not ensure its validity against the [Citation
File Format
schema](https://github.com/citation-file-format/citation-file-format/blob/main/schema-guide.md):
...
[`cff_validate()`] reports validation problems and invisibly returns `TRUE` or
`FALSE`.
...
## Base methods provided by \CRANpkg{cffr}

\CRANpkg{cffr} provides additional S3 methods for common coercion functions from
the base and utils namespaces.
