...
To map general BibTeX entries to CFF key `type`, the following equivalence is
proposed:
...
- For the **address** field, its intended use in BibTeX varies depending on the
  entry type. For **\@inproceedings**, it denotes the **address** of the
...
- **Lossy mappings**: Some BibTeX fields, including **crossref**, **annote** and
...
- **Ambiguous semantics**: Certain fields such as **address** and **series**
  have context-dependent meanings in BibTeX, requiring heuristic mapping to CFF
  keys such as `institution`, `location` or `collection-title`.
...
- **Non-reversibility**: Because of these factors, round-trip conversion
  (BibTeX → CFF → BibTeX) is not guaranteed to preserve the original structure
  or semantics.
