...
works. An example structure of a **BibTeX** entry is:
...
The final text representation of the entry is shown as[^1]:
...
capabilities for reading and transforming **BibTeX** into different formats:

-   `cff_read_bib()` reads \*.bib files.
-   `cff_read_bib_text()` reads **BibTeX** entries stored in a variable.
-   An S3 method `toBibtex.cff()` that converts `cff` objects to **BibTeX**
...
values[^5] for the [type]{.underline} field, the following conversion is applied
...
-   **series** is mapped to [collection-title]{.underline} only on those
    entries that do not require **booktitle**. In practice, this means that
...
prescribe an **author** field. In these cases, as [authors]{.underline} is required on [CFF]{.underline},
we use *anonymous*[^7] when converting to [CFF]{.underline} and omit it when
converting from [CFF]{.underline} to **BibTeX**.
...
[^1]: By default **R** Pandoc generates the cite in the Chicago author-date
