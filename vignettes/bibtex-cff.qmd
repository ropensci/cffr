...
abstract: >
  ...
  expressiveness, the mapping is not bijective, but it enables practical
  interoperability.
link-citations: true
...
storing references, originally designed for document-centric workflows in 1985.
It represents bibliographic entries as relatively flat records relying on
conventions and free-text fields.

The Citation File Format (CFF) [@druskat_citation_2021] provides a structured
and extensible way to represent citation metadata, particularly for software and
research outputs. It supports explicit typing, nested objects, and richer
metadata.

In modern workflows, both formats often coexist. BibTeX remains dominant in
academic publishing, while CFF is adopted by platforms such as GitHub and
Zenodo.

The **cffr** package [@hernangomez2021] provides a deterministic mapping
(crosswalk) between BibTeX and CFF, enabling practical interoperability.

Due to fundamental differences in their data models, this mapping is not
bijective. Mappings may involve structural transformations, heuristic parsing,
and controlled information loss.

## Conceptual differences between BibTeX and CFF

BibTeX represents bibliographic entries as flat records with loosely structured
fields.

CFF, in contrast, defines a structured schema with explicit typing, nested
objects, and support for richer metadata such as contributor roles.

As a result, direct one-to-one mappings are not always possible. Mappings
require a combination of direct mappings, structural transformations, and
heuristic parsing.
...
- `references`: Includes reference(s) to other creative works related to the
  ...
  these references may encompass papers describing the abstract concepts of the
  software or algorithms implemented in the software version.
...
These keys correspond conceptually to **BibTeX** [fields](#fields), with the
exception of the key [type]{.underline}. In [CFF]{.underline}, this key defines
the type of work[^1], making it analogous to the **BibTeX** [entries](#0).
...
2.  Apply mapping rules to transform types and fields
3.  Serialize the result into CFF (and vice versa)

The mapping is deterministic whenever possible, but some fields require
heuristic parsing.
...
The resulting object converts flat BibTeX fields into structured CFF metadata,
normalizing field names and representations where required.

## Mapping tables

The following tables summarize the mapping rules implemented in **cffr**.

Mapping types:

- **direct**: preserved without modification.
- **transform**: structural or naming transformation.
- **split**: one field mapped to multiple fields.
- **heuristic**: requires parsing or inference.
- **unsupported**: dropped during mapping.
- **enrichment**: only exists in CFF.

These mappings are not bijective and may introduce normalization effects.

::: {#tbl-mapentry}
| BibTeX type | CFF [type]{.underline} | Mapping type | Notes |
|:---|:---|:---|:---|
| **\@article** | [article]{.underline} | direct | \- |
| **\@book** | [book]{.underline} | direct | \- |
| **\@inbook** | [chapter]{.underline} | transform | Normalized to CFF taxonomy |
| **\@incollection** | [chapter]{.underline} | transform | Same as **\@inbook** |
| **\@manual** | [manual]{.underline} | direct | \- |
| **\@misc** | [generic]{.underline} | heuristic | Fallback when no better match |
| **\@phdthesis** | [thesis]{.underline} | transform | The specific thesis subtype (e.g., master’s vs PhD) may be lost when mapping from BibTeX to CFF if the original entry does not include a [type]{.underline} field |
| **\@techreport** | [report]{.underline} | transform | \- |
| **other** | [generic]{.underline} | heuristic | Default fallback |

: Entry types
:::

::: {#tbl-mapcore}
| BibTeX field | CFF field | Mapping type | Notes |
|:---|:---|:---|:---|
| **title** | [title]{.underline} | direct | \- |
| **author** | [authors]{.underline} | transform | Parsed into structured name objects |
| **editor** | [editors]{.underline} | transform | Same parsing logic as authors |
| **year** | [year]{.underline} | direct | \- |
| **month** | [month]{.underline} | heuristic | Format normalization required |
| **journal** | [journal]{.underline} | direct | \- |
| **booktitle** | [collection-title]{.underline} | transform | Container title |
| **volume** | [volume]{.underline} | direct | \- |
| **number** | [issue]{.underline} | transform | Naming normalization |
| **pages** | [start]{.underline} / [end]{.underline} | split | Parsed from page range |
| **doi** | [doi]{.underline} | direct | \- |
| **url** | [url]{.underline} | direct | \- |

: Core fields
:::

::: {#tbl-mapstruct}
| BibTeX field(s) | CFF field | Mapping type | Notes |
|:---|:---|:---|:---|
| **publisher** | [publisher.name]{.underline} | transform | Converted to structured object |
| **address** | [publisher.location]{.underline} | transform | Combined with publisher |
| **institution** | [institution]{.underline} | direct | Used mainly in reports/thesis |
| **school** | [institution]{.underline} | transform | Thesis normalization |
...
| BibTeX field | CFF field | Mapping type | Notes |
|:---|:---|:---|:---|
| **pages** | [start]{.underline}/[end]{.underline} | heuristic | Requires parsing, may fail |
| **author** | [authors]{.underline} | heuristic | Name splitting is not always reliable |
| **month** | [month]{.underline} | heuristic | Multiple formats possible |
| **note** | [notes]{.underline} | heuristic | Free text |

: Parsed fields
:::

::: {#tbl-mapnobib}
| CFF field | BibTeX equivalent | Mapping type | Notes |
|:---|:---|:---|:---|
| [preferred-citation]{.underline} | \- | enrichment | CFF-specific concept |
| [references]{.underline} | \- | enrichment | Not representable |
| [type: software]{.underline} | \- | unsupported | No BibTeX equivalent |
| [identifiers]{.underline} | partial | transform | Only DOI/URL mapped |

: CFF fields with no BibTeX equivalence
:::

## Proposed Crosswalk

The **cffr** package [@hernangomez2021] provides utilities for mapping
**BibTeX** entries (via the **R** base function `bibentry()`) to
[CFF]{.underline} files and vice versa. This section describes how the mapping
between both formats has been implemented. The crosswalk is partially based on
@Haines_Ruby_CFF_Library_2021[^2].
...
For mapping general **BibTeX** entries to [CFF]{.underline} [types]{.underline},
the following crosswalk is proposed:
...
- **\@book**, **\@inbook**, and **\@incollection** are closely related in
  **BibTeX**[^3]. While **\@inbook** and **\@incollection** both reference parts
  of a **\@book**, the former is used for citing sections, chapters, pages, or
  other specific parts, whereas the latter is used for citing parts with a
  specific title. Since [CFF]{.underline} allows keys `type:book` and
...
- **\@mastersthesis** and **\@phdthesis** are tagged using a combination of
  `type:thesis` and `thesis-type`.

Additionally, considering that [CFF]{.underline} allows for a wide range of
values[^4] for the [type]{.underline} field, the following mapping is applied
...
- Some fields are not mapped because there is no clear equivalence with
  ...
  corresponds to the identifier of the work (similar to an entry in **BibTeX**),
  therefore, **BibTeX type** won't be mapped. These fields are always optional
  in **BibTeX**.

- For the **address** field, its intended use in **BibTeX** varies depending on
  the entry type (e.g., for **\@inproceedings**, it denotes the **address** of
  the **conference**, while for **\@mastersthesis/\@phdthesis**, is the
  **address** of the **school**, etc.). Mapping between **BibTeX** and
  [CFF]{.underline} becomes more complex when related to institutions, resulting
  in varying final mappings in [CFF]{.underline}. When mapping from
  [CFF]{.underline} to **BibTeX**, we propose to follow the same entry-based
  logic, using the key [location]{.underline} as a fallback value when mapping
  to **address**.

- In relation with this complexity mentioned above, **institution,
  organization** and **school** are mapped to [institution]{.underline}.

- **series** is mapped to [collection-title]{.underline} only on those entries
  that do not require **booktitle**. In practice, this means that
  collection-title corresponds to **booktitle** for **incollection** and
  **inproceedings**, and in the other cases it corresponds to series. As a
  consequence, series information is lost for incollection and inproceedings,
  but in those cases it is an optional field.

- When mapping from [CFF]{.underline} to **BibTeX**, we propose to use
  [date-published]{.underline} as a fallback for extracting **month** and
  **year** fields.

- When **pages** is provided as a range separated by `--`, i.e. **pages =
  {3--5}**, it is coerced as [start: 3]{.underline}, [end: 5]{.underline} in
  [CFF]{.underline}.

#### BibLaTeX

Additionally, there are other [CFF]{.underline} keys that correspond to
**BibLaTeX** fields. We propose to include these fields in the crosswalk[^5],
...
The mapping between BibTeX and CFF has several limitations.

### Bijectivity and Round-tripping

Mapping from BibTeX to CFF and back does not guarantee identical output.

### Information loss

- Nested CFF structures are flattened in BibTeX.
- Contributor roles are not preserved.
- CFF-specific types (e.g., software) have no equivalent.

### Heuristic parsing

- Author names require parsing.
- Page ranges must be split.
- Month formats vary.

### Unsupported fields

Some BibTeX fields (e.g., `howpublished`, `annote`, `key`) are not represented
in CFF.

### Normalization

Field values may be normalized, resulting in differences in formatting.

## Design decisions

The mapping implemented in **cffr** follows these principles:

- Prefer deterministic mappings when possible.
- Use heuristics only when necessary.
- Normalize fields to align with CFF conventions.
- Favor interoperability over exact round-trip fidelity.
...
[publisher]{.underline}'s [address]{.underline}. Finally, the key
[collection-type]{.underline} is populated with [book-series]{.underline}.
...
Additionally, **series** is ignored because there is no clear mapping on
...
as required on CFF, so it is ignored to avoid validation errors.
...
[thesis-type]{.underline}. If [thesis-type]{.underline} contains a [regex
pattern](https://regex101.com/r/mBWfbs/1) `(?i)(phd)`, it is recognized as
**\@phdthesis**.

Additionally, **school** is mapped to [institution]{.underline}.
...
prescribe an **author** field. In these cases, as [authors]{.underline} is
required on [CFF]{.underline}, we use *anonymous*[^6] when mapping to
[CFF]{.underline} and omit it when mapping from [CFF]{.underline} to **BibTeX**.
...
While the formats are not fully equivalent, the mapping implemented in **cffr**
provides a consistent and transparent transformation between them.

By making mapping rules explicit and documenting limitations, this work supports
interoperability between legacy bibliographic systems and modern software
citation practices.
...
[^1]: See a complete list of possible values of [CFF]{.underline}
    [type]{.underline} in [Appendix B](#appendix_cff_type).

[^2]: Note that this software performs only the mapping from [CFF]{.underline}
    to **BibTeX**, however **cffr** can perform the mapping in both directions.

[^3]: Note that **BibLaTeX** [@biblatexpack] handles **\@inbook** differently,
