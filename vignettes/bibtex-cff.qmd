...
remains dominant in academic publishing pipelines, while CFF is increasingly
adopted by infrastructure platforms such as **GitHub** and **Zenodo**.
...
differences in their data models, this mapping is not bijective and may require
...
defined fields, optimized for citation rendering in documents. Semantics are
largely implicit and contextual, depending on entry types and bibliography
styles.
...
@patashnik1988 provides the canonical description of the **BibTeX** format. Two
key concepts are central: **entries** and **fields**.

### Entries {#entries}

The original **BibTeX** specification defines fourteen canonical entry types,
each corresponding to a class of cited work:
...
Other implementations, notably **BibLaTeX** [@biblatexpack], extend this set
with additional entry types such as online resources, software and datasets. In
...
**BibTeX** entries consist of fields whose relevance and requirements depend on
the entry type. Some fields are mandatory, some are optional and others are
ignored by standard bibliography styles while still carrying useful information.

The following table summarizes the relationship between **BibTeX** **entries**
and their **required or optional fields**, following @patashnik1988. Required
fields are marked with **x** and optional fields with **o**.
...
heuristic parsing, especially when normalizing names, dates or page ranges. As a
result, semantic round-trip reversibility is not guaranteed, even when the
...
::: callout-note
For clarity throughout this document, **bold** formatting (for example,
**\@book** and **edition**) is used for **BibTeX** entries and fields, whereas
inline code formatting (for example, `book` and `edition`) is used for CFF keys.
:::
...
**cffr** provides utilities for mapping **BibTeX** entries (with `bibentry()`)
to CFF and back. This section describes how the crosswalk is implemented,
partially informed by the **Ruby** implementation in
...
models**](#entrymodels) that refine these rules for specific **BibTeX** entry
types.
...
- **Direct mapping**: one-to-one correspondence between a **BibTeX** field and a
  CFF key.
...
To map general **BibTeX** entries to CFF key `type`, the following equivalence
is proposed:
...
- **\@book**, **\@inbook** and **\@incollection** are closely related in
  **BibTeX**[^3]. The crosswalk maps **\@book** and **\@inbook** to `type: book`
  and distinguishes **\@inbook** with `section` and/or `start`/`end`.
  **\@incollection** maps to `type: generic`, with **booktitle** preserved as
  `collection-title`. When **series** is provided for **\@book** or
  **\@inbook**, it is represented by `collection-title` together with
...
- Some fields are not mapped because they have no clear equivalent CFF keys
  ...
  entry in **BibTeX**. Therefore, **BibTeX** **type** is not mapped. These
  fields are always optional in **BibTeX**.
- For the **address** field, its intended use in **BibTeX** varies depending on
  the entry type. For **\@inproceedings**, it denotes the **address** of the
  **conference**, while for **\@mastersthesis/\@phdthesis**, it is the
  **address** of the **school**. Mapping between **BibTeX** and CFF becomes more
  complex when institutional metadata is involved. This results in varying final
  mappings in CFF. When mapping from CFF to **BibTeX**, we propose to follow the
  same entry-based logic, using the key `location` as a fallback value when
  mapping to **address**.
- Because of the complexity mentioned above, **institution, organization** and
  **school** are mapped to `institution`. For applicable entry types,
  **organization** or **school** takes precedence when present, and otherwise an
  existing **institution** is retained.
- A **url** is preserved as `url`. URLs using the `doi.org` or `dx.doi.org`
  resolver also populate `doi` when their paths contain a valid DOI value. Each
  URL is classified independently when an entry provides multiple URLs.
...
- When mapping BibLaTeX **date** to CFF, complete dates populate
  `date-published`, while `YYYY` and `YYYY-MM` values populate the available
  **year** and **month** fields without inventing a day. Date values that cannot
  be represented remain unchanged so that CFF validation can report them.
- When **pages** is provided as a range separated by `--`, that is, **pages =
...
- **Lossy mappings**: Some **BibTeX** fields, including **crossref**, **annote**
  and **key**, have no equivalent in CFF and are omitted during conversion.
- **Ambiguous semantics**: Certain fields such as **address** and **series**
  have context-dependent meanings in **BibTeX**, requiring heuristic mapping to
  CFF keys such as `institution`, `location` or `collection-title`.
- **Type collapsing**: Multiple **BibTeX** entry types, including **\@misc** and
...
- **Structural transformations**: Some fields require transformation rather than
  direct mapping, such as **pages** → `start`/`end`, altering the representation
...
[xampl.bib](https://tug.org/texmf-docs/bibtex/xampl.bib) distributed with
**BibTeX** [@patashnik].
...
**page** field, while **\@book** does not allow these fields as optional.
...
**\@inbook** entry (discussed further in [Appendix A](#appendix_inbook)). We
propose treating a **BibLaTeX** **\@inbook** as a **BibTeX** **\@incollection**.
...
The **BibTeX** field requirements are identical for **\@mastersthesis** and
**\@phdthesis**.
...
The crosswalk for **\@misc** uses the default mapping rules. This **entry** does
not require any **field**.

Note also that it is mapped to `type: generic`, as with
[**\@incollection**](#incol). In this case, **booktitle** is not an option, so
the proposed definition should cover both **\@misc** and **\@incollection**.
...
prescribe an **author** field. In these cases, because `authors` is required in
CFF, we use *anonymous*[^6] when mapping to CFF and omit it when mapping from
CFF to **BibTeX**.
...
This article presents a practical and reproducible crosswalk between **BibTeX**
and CFF.
...
requirement in **BibLaTeX**. Notably, **BibTeX** **\@incollection** also
requires this field. Moreover, both **BibTeX** **\@incollection** and
...
**BibTeX**. Therefore, we treat a **BibLaTeX** **\@inbook** entry as equivalent
to a **BibTeX** **\@incollection**, given the close alignment in their
