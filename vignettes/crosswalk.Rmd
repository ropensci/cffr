...
```{r authors}
# Default behaviour
cff_obj <- cff_create("jsonvalidate")
pkg <- desc::desc(file.path(find.package("jsonvalidate"), "DESCRIPTION"))
...
# Creators and Copyright Owners only
cff_obj_cre_cph <- cff_create("jsonvalidate", authors_roles = c("cre", "cph"))
cff_obj_cre_cph$authors
length(cff_obj_cre_cph$authors)

# Check
authors[vapply(authors, function(x) {
  "cre" %in% x$role || "cph" %in% x$role
}, logical(1))]
```
