...
-   `cff_create_bib_text()` can read **BibTeX** entries that are already stored
    in a variable.
-   A S3 method `toBibtex.cff()` that converts from
...
# To cff
library(cffr)
cff_format <- cff_create_bib_text(string)
...
cff_create_bib_text(bib)
```

From [CFF]{.underline} to **BibTeX**

```{r echo=FALSE}
toBibtex(cff_create_bib_text(bib))
```
