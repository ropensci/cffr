...
-   `cff_read_biblines()` can read **BibTeX** entries that are already stored in
    a variable.
...
# To cff
library(cffr)
cff_format <- cff_read_biblines(string)
...
cff_read_biblines(bib)
```

From [CFF]{.underline} to **BibTeX**

```{r echo=FALSE}
toBibtex(cff_read_biblines(bib))
```
