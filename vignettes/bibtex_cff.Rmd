...
thisart <- bibentry("Article",
  title = "{BibTeX} and {CFF} using",
  key = "hernangomez2022",
  author = "Diego Hernangómez",
  journal = "The {cffr} package",
  year = 2022,
  volume = "Vignettes",
  abstract = "An (un) opinionated crosswalk between {BibTeX} and {CFF}. This article presents a crosswalk between BibTeX and Citation File Format, as
  it is performed by the cffr package (R software)."
)
cat("  \n")
thisart
...
Regarding the entries, `bibentry()` does not implement the **\@conference**
entry. However, we can replace that key by **\@inproceedings** given that the
definition is identical:

```{r try-conf}

try(bibentry("Conference",
  key = "conference",
  author = "Peter Draper",
...
# Replace by InProceedings
inpr <- bibentry("InProceedings",
  key = "conference",
  author = "Peter Draper",
  title = "The title of the work",
  booktitle = "The title of the book",
...
There is a strict relation between Entries and Fields on BibTeX. Dependending on
the type of entries, some fields are required while others are optional or even
ignored. On the following table, required field are flagged as **r** and
optional fields are flagged as **o.** Fields with an asterisk (**\***) denotes
that there are some degree of optionality on the requirement of the field, see
...
[Entries](#entries). The rest are optional and, strictly speaking, not necessary
if no listed on the optional field of *Table 1*.
...
```{r refkeys, results='asis', message=FALSE, warning=FALSE}
library(cffr)


refkeys <- matrix(cff_schema_definitions_refs(), ncol = 7, byrow = TRUE)
...
These keys are the equivalent to the [fields](#fields) of BibTeX, with the
exception of the key **type**. On CFF, this key defines the type of work[^5],
therefore this is the equivalent to the BibteX [entries](#entries).
...
## Proposed crosswalk between BibTeX and CFF
...
As there is not a perfect one-by-one relationship between both formats, it is
needed to provide two different approaches: one from the conversion from BibTeX
to CFF and another for conversions from CFF to BibTeX
...
```{r entry_b2c, echo=FALSE, comment=""}
...
article <- bibentry("Article",
  ...
  author = "Maritza Arganis and Rafael Val and Jordi Prats and Katya Rodriguez and Ramon Dominguez and Josep Dolz",
...
Would be converted to the following CFF format

```{r echo=FALSE, comment=""}
cff_parse_citation(article)
```

And an **\@inproceedings** entry:

```{r echo=FALSE, comment=""}
inpr <- bibentry("InProceedings",
  author = "A. Demir and E. Cilden and F. Polat",
...
: Fields: BibTeX/CFF direct mapping
