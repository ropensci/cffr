...
Additionally, the **cffr** package [@hernangomez2021] incorporates a function
`cff_from_bibtex()` that can be used to read and transform **BibTeX** strings
...
citation_format <- cff_to_bibentry(cff_format)
...
13. **month**: Indicates the month in which the work was published or, for an
    ...
    abbreviation (e.g., `jan`, `feb`, `mar`) as described in Appendix B.1.3 of
    the LaTeX book [@lamport86latex].
...
fields are flagged as **o**. Fields enclosed in parentheses (**(x), (o)**)
indicate varying degrees of flexibility regarding the field requirements. For
more detailed information, refer to @patashnik1988.
...
[^2]: See a complete list of possible values of [CFF]{.underline} type on the
    [Guide to Citation File Format schema version
...
It is important to note that this Entry/Type crosswalk is intended exclusively
for **BibTeX**. This is relevant since another tools based on
[Biber](https://en.wikipedia.org/wiki/Biber_(LaTeX)) such as **BibLaTeX**
handles some entries differently, most notably the **\@inbook** entry. We
provide additional information and support on a dedicated
[Annex](#annex_inbook).

Additionally, considering that [CFF]{.underline} allows for a wide range of
values[^4] for the [type]{.underline} field, the following conversion would be
applied from [CFF]{.underline} to **BibTeX**:

[^4]: See the [Appendix](#appendix_cff_type) for all possible values.
    Information extracted from @druskat2019.
...
There exists a significant similarity between the definitions and names of
certain **BibTeX fields** and [CFF]{.underline}. In the following cases, the
equivalence is nearly straightforward:

```{r fields_bib2cff, echo=FALSE,results='asis'}
df_table <- table_master[table_master$table == "fields_bib2cff", c(2:3)]
df_table[is.na(df_table)] <- ""
# fix links
...
knitr::kable(df_table,
  col.names = c("**BibTeX Field**", "[CFF key]{.underline}"),
...
Note that the **BibTeX** fields marked with **"\*"** on each table indicate
those that are strictly required for each entry type.
...
```{r echo=FALSE}
toBibtex(cff_to_bibentry(cff_from_bibtex(bib)))
```

### \@book / \@inbook {#book-inbook}

In terms of the fields required in **BibTeX**, the primary difference between
**\@book** and **\@inbook** lies in the latter's additional demand for a chapter
or page specification. While **\@book** does not even allow these fields as
optional, **\@inbook** requires them. Therefore, we propose that an **\@inbook**
entry in [CFF]{.underline} be treated as a **\@book** with the following
supplementary fields:

1.  [section]{.underline}: To denote the specific section within the book.
...
As mentioned previously, there are notable differences in how **BibTeX** and
**BibLaTeX** handle the **\@inbook** entry. This is further discussed in the
[Annex](#annex_inbook).
...
Additionally, **series** would be ignored as there is not clear mapping on [CFF]{.underline} for this field.
...
| **address**        | [address]{.underline} property of [publisher]{.underline} | See **Note** on [\@book / \@inbook](#book-inbook) |
...
| BibTeX                           | [CFF]{.underline}                                           | Note                                                                |
|--------------------|-----------------------------|-------------------------------|
| **\@mastersthesis, \@phdthesis** | [type: thesis]{.underline}                                  | Use also [thesis-type]{.underline} for identifying the thesis type. |
| **author\***                     | [authors]{.underline}                                       |                                                                     |
| **title\***                      | [title]{.underline}                                         |                                                                     |
| **school\***                     | [institution]{.underline}                                   |                                                                     |
| **year\***                       | [year]{.underline}                                          |                                                                     |
| **type**                         |                                                             |                                                                     |
| **address**                      | [address]{.underline} property of [institution]{.underline} | See **Note** on [\@book / \@inbook](#book-inbook)                   |
| **month**                        | [month]{.underline}                                         | See **Note** on [article]                                           |
| **note**                         | [notes]{.underline}                                         |                                                                     |
...
Note also that it is mapped to [type: generic]{.underline} as [incollection],
...
[authors]{.underline} is required on CFF, we would use *anonymous*[^6] when
...
## Annex: **\@inbook** in Bibtex and BibLaTeX {#annex_inbook}

TODO....

## Appendix: [CFF key:type]{.underline} values {#appendix_cff_type}
