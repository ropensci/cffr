...
[r-universe](https://r-universe.dev/search) or with packages such as **remotes**
and **pak**.
...
If the package source code is hosted on GitHub, **cffr** can also retrieve the
topics of the repository with the [GitHub API](https://docs.github.com/en/rest)
and include those topics as keywords. This option is controlled with the
...
**CRAN** or [r-universe](https://r-universe.dev/search). For packages without
this field in the `DESCRIPTION` (which is the typical case for an in-development
package), **cffr** tries to find the package in any of the default repositories
specified in `options("repos")`.
