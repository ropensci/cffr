...
This section summarizes the keys that **cffr** can populate and the metadata
source for each one. Details for each key are presented in the next section. The
assessment is based on the [Guide to Citation File Format schema version
1.2.0](https://github.com/citation-file-format/citation-file-format/blob/main/schema-guide.md#valid-keys)
...
Set this key with the `cff_version` argument of `cff_create()` or `cff_write()`:
...
This field is present in packages installed from
[**r-universe**](https://r-universe.dev/search) or with package managers such as
**remotes** and **pak**.
...
- `"Date"`, when present.
- Otherwise, `"Date/Publication"`. This field is present in packages built on
...
**cffr** removes identical entries after merging identifiers from these sources.
...
removed. A single keyword is preserved as a one-item CFF `keywords` array.
...
If the package source code is hosted on **GitHub**, **cffr** can also retrieve
repository topics with the [**GitHub** API](https://docs.github.com/en/rest) and
include those topics as keywords. This option is controlled with the
`gh_keywords` argument:
...
This key is extracted from the `"License"` field of the `DESCRIPTION` file. All
alternatives recognized by the `cran_to_spdx` mapping are preserved.
...
the remaining citations as [references](#references). If the file cannot be read
with the package metadata, **cffr** does not retry with metadata from another
package.
...
calling `citation(auto = TRUE)` for each one. It preserves the citation
metadata, including the title, abstract, authors and year, while adapting the
reference to the CFF software type. It adds the dependency scope and version
constraint from the source `DESCRIPTION`, URLs and repository information from
the installed dependency's `DESCRIPTION` and the canonical **CRAN** DOI only
when the dependency is available from **CRAN**. A release date, when available,
determines the year, with the year returned by `citation()` retained otherwise.
The year is omitted only when neither source provides one.
...
This field is usually populated when a package is hosted in a repository such as
**CRAN** or [**r-universe**](https://r-universe.dev/search). For packages
...
[**Bioconductor**](https://bioconductor.org/) packages are identified when the
...
This key combines the `"Package"` and `"Title"` fields of the `DESCRIPTION`
file.
...
This key has the fixed value `"software"`. The other valid value is `"dataset"`.
See the [Guide to CFF schema
