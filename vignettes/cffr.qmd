...
You can customize a `cff` object (a custom **cffr** S3 class) using the
coercion system provided in the package and the `keys` argument.

For demonstration, we create a `cff` object using `cff()` and then add or
modify its contents.
...
The process is similar to the example with `person`: we modify our
`cff` object:
