...
![GitHub citation support announcement](vignettes/tweet-1.png){.mx-auto .d-block
...
**codemetar** [@codemetar2021] allow for generating `codemeta.json` files from
**R** package metadata.

## The cffr package

**cffr** maximizes data extraction by using both the `DESCRIPTION` file and the
`CITATION` file (if present) from your package. Note that **cffr** works best if
your package passes `R CMD check/devtools::check()`.
...
cat("\n")
today <- Sys.Date()
# Try to get the count of GitHub repositories here.
...
you to customize the `CITATION.cff` and integrate it into your workflows.

This basic example shows how to create a `cff` object (see `?cff` for more
information). In this case, we are creating a `cff` object from the metadata of
...
information in a `CITATION.cff` created with **cffr**. See [About CITATION
